...
export default function Home() {
  ...
  const [tempIterations, setTempIterations] = useState<number>(100);
  ...
  useEffect(() => {
    ...
    julia();
  }, [cx, cy, tempIterations, paletteNumber, threshold, canvasHeight, canvasWidth, juliaWindow]);

  useEffect(() => {
    console.log('rendering mandelbrot', mandelbrotWindow)
    mandelbrot();
  }, [tempIterations, paletteNumber, threshold, canvasHeight, canvasWidth, mandelbrotWindow]);
      ...
      if (ctx !== null) {
        ...
        for (let iy = 0; iy < canvasHeight; iy++) {
          ...
          for (let ix = 0; ix < canvasWidth; ix++) {
            ...
            const theIterations = computePoint(currentPoint, cx, cy, tempIterations, threshold);
  ...
  /*function computePoint(point: {x: number; y: number}, cx: number, cy: number) {
    ...
    let i = 0
    while ((i < tempIterations) && ((x2 + y2) < threshold)) {
        let temp = x2 - y2 + cx
  ...
  function setColourUsingLevelSetMethod(theIterations: number, ctx: any) {
    if (theIterations == tempIterations) { // we are in the set
        ctx.fillStyle = "#000"
    } else {
  ...
  return (
    <Page>
      ...
      <Input
        ...
        step={25}
        onChange={(value) => setTempIterations(value.target.valueAsNumber)}
      /></Label>
