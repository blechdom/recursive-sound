...
/*
	...
	function doSave()
	{
		const getSpiralSvgFile = ( s, svg ) =>
			s.serializeToString( svg ).split( '\n' );

		const spiralSvg = getSpiralTilingSVG();
		console.log("svg ", spiralSvg);
		const spiralSvgFile = getSpiralSvgFile( new XMLSerializer(), spiralSvg );
		console.log("spiralSvgFile", spiralSvgFile);
		p5c.save( spiralSvgFile, "spiral", "svg" );
	...
	*/
		...
		for ( let i = i_i; i <= i_f; i++ ) {
			for ( let j = j_i; j <= j_f; j++ ) {
				...
				console.log("i, j, v, vp, s, r", i, j, v, vp, s, r);
